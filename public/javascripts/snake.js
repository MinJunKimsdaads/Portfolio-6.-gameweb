...
window.addEventListener("DOMContentLoaded", function (event) {
    ...
    const color = "gray"; // Primary color
    ...
    function resetGame() {
      ...
      // Reset header
      contrastElement.innerText = `${Math.floor(contrast * 10)}s`;
    ...
    window.addEventListener("keydown", function (event) {
      ...
      if (event.key == "H" || event.key == "h") {
        ...
        noteElement.innerHTML = `하드모드, 스페이스를 눌러 게임을 시작하세요`;
      ...
      if (event.key == "E" || event.key == "e") {
        ...
        noteElement.innerHTML = `이지모드, 스페이스를 눌러 게임을 시작하세요`;
        noteElement.style.opacity = 1;
        resetGame();
        return;
      }
    ...
    function main(timestamp) {
      try {
        ...
        const pressSpaceToStart = "스페이스바를 눌러 리셋하세요";
    ...
    function stepAndTransition(percentageOfStep) {
      ...
      if (newHeadPosition != applePosition) {
        ...
        const tailValue = `${100 - percentageOfStep * 100}s`;
    ...
    function getNextPosition() {
      ...
      switch (snakeDirection) {
        ...
        case "up": {
          const nextPosition = headPosition - width;
          if (nextPosition < 0) throw Error("뱀이 벽에 부딪쳤습니다");
          // Ignore the last snake part, it'll move out as the head moves in
          if (snakePositions.slice(1).includes(nextPosition))
            throw Error("뱀이 자기 꼬리를 물었습니다");
          return nextPosition;
